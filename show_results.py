...
def plot_advancement(df):
    ...
    width = 0.35
    ax.barh(y + width / 2, df["local_advance_pct"], width, label="Local", color="#2ecc71")
    ax.barh(y - width / 2, df["away_advance_pct"], width, label="Visitante", color="#e74c3c")
    ax.set_yticks(y)
    ...
    for i, (l, a) in enumerate(zip(df["local_advance_pct"], df["away_advance_pct"])):
        ax.text(l + 1, i + width / 2, f"{l:.0f}%", va="center", fontsize=7, color="#2ecc71")
        ax.text(a + 1, i - width / 2, f"{a:.0f}%", va="center", fontsize=7, color="#e74c3c")
    fig.tight_layout()
...
def plot_match_probabilities(df):
    n = len(df)
    fig, axes = plt.subplots(n // 4 + 1, 4, figsize=(14, 3 * (n // 4 + 1)))
    axes = axes.flatten()
