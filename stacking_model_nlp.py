...
def main():
    print("=" * 60)
    print("STACKING MODEL + NLP - 16avos World Cup 2026")
    print("=" * 60)

    # Check NLP data exists before proceeding
    import os
    if not os.path.exists(NLP_FEATURES):
        print("\n  ERROR: NLP features file not found.")
        print("  ************************************************************")
        print("  Para usar el modelo NLP necesitas generar los datos primero.")
        print()
    ...
    print("\n[2] Building feature vectors (base features only)...")
    ...
    print("\n[4] Predicting 16avos matchups with NLP features...")
    ...
    for i, match in enumerate(matches, 1):
        ...
        print(f"   NLP: {match['local']}  {pc_str_h}  ...  YT={fv.get('home_yt_sent',0):+.3f}")
        print(f"        {match['visitante']}  {pc_str_a}  ...  YT={fv.get('away_yt_sent',0):+.3f}")
    ...
    print(f"\n   {'=' * 60}")
    pl.DataFrame(predictions_rows).write_csv("data/knockout_predictions_nlp.csv")
    print("   Predictions saved to data/knockout_predictions_nlp.csv")
